	...
	protected void notifyListeners(JsonObject message) throws InvalidMessageException{
		String msgCode = message.get(ConnectionConsts.MESSAGE_C_MESSAGE_CODE).getAsString();
  		...
  		if(msgCode.equals(ConnectionConsts.SERVER_C_AUTH_RETURN)){
	  		...
	  		String _newPermission = obj.get(ConnectionConsts.MESSAGE_C_ALLOWED_OPS).getAsString();
	  		...
	  		JsonObject _sourceList = obj.getAsJsonObject(ConnectionConsts.MESSAGE_C_SOURCE_LIST);
	  		if(_sourceList != null )
	  			_sources = sourceList.entrySet();
	  		...
	  		String _permission = obj.get(ConnectionConsts.MESSAGE_C_ALLOWED_OPS).getAsString();
